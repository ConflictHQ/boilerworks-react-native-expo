...
| Yarn 4 (Berry), not npm                                             | Workspace-ready, faster installs, better caching. `.yarn/` caches are gitignored (`install-state.gz`, `cache/`). `packageManager` field in `package.json` enforces it.                              |
